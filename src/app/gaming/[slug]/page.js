...
export function generateStaticParams() {
  return getSlugs("gaming").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const { slug } = await params;
  const p = await getEntry("gaming", slug);
  if (!p) return { title: "Gaming" };
  const url = `/gaming/${slug}`;
  return {
    title: p.title,
...
export default async function GamingPost({ params }) {
  const { slug } = await params;
  const p = await getEntry("gaming", slug);
  if (!p) notFound();
  ...
  return (
    <PageShell title={p.title} subtitle={p.summary} compact>
      ...
      <Link href="/gaming" className="back-link">
